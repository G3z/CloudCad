S.define(
    "views/draw/2D/primitives/Path"
    ["views/draw/2D/primitives/AbstractPrimitive", "views/draw/2D/primitives/Pont2D", "views/draw/3D/primitives/Segment"]
    (AbstractPrimitive, Point2D, Segment)

        class Path extends AbstractPrimitives
            @paperPath
            @points
            @segments
            @lastPoint
            ...
            arrayToPoint:(point)=>
                if point instanceof Array
                    if point.length == 2
                        if @name?
                            return point = new Point2D(point[0],point[1],name + points.length,this) 
                        else
                            return point = new Point2D(point[0],point[1],null,this)
