...
S.define(
    "views/draw/2D/primitives/Line"
    ["views/draw/2D/primitives/AbstractPrimitive", "views/draw/2D/primitives/Pont2D"]
    (AbstractPrimitive, Point2D)->

        class Line extends AbstractPrimitives
            constructor:(@start,@end,@name)->
                ...
                if @start instanceof Array
                    if @start.length == 2
                        if @name?
                            @start = new Point2D(@start[0],@start[1],name + "0",this) 
                        else
                            @start = new Point2D(@start[0],@start[1],null,this) 

                if @end instanceof Array
                    if @end.length == 2
                        if @name?
                            @end = new Point2D(@end[0],@end[1],name + "1",this) 
                        else
                            @end = new Point2D(@end[0],@end[1],null,this) 
