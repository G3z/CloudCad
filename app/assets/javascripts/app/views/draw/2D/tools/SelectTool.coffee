...
S.define(
    "views/draw/2D/tools/SelectTool"
    ["views/draw/2D/Tool2D", "views/draw/2D/primitives/Pont2D" , "views/draw/2D/primitives/Path" ]
    (Tool2D, Point2D, Path)->
        
        class SelectTool extends Tool2D
            @selectedIdx
            constructor:(stage2d)->
                super(stage2d)
                @selectedIdx=null
            ...
            mouseDown:(eventpoint)=>
                mousePoint = new Point2D(@stage2d.mouse.currentPos.x, @stage2d.mouse.currentPos.y,"")
                #debugger
                ...
                if hitTest?
                    if @path? && @path.id != hitTest.item.ccObject.id
                        ...
                        if hitTest.item.ccObject instanceof Path
                            @path = hitTest.item.ccObject
                            @path.selected(true)
                            for segment in @path.paperPath.segments
                                segment.selected = true
