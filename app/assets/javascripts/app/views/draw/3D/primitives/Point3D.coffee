class CC.views.draw.primitives.Point3D extends CC.views.draw.primitives.AbstractPrimitives
    @x
    @y
    @name
    @father
    ...
    moveTo:(@x,@y,@z)=>
        if @father?
            if @father instanceof CC.views.draw.primitives.Path3D
    ...
    remove:->
        if @father? && @father instanceof CC.views.draw.primitives.Path3D
