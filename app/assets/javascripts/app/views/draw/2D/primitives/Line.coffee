class CC.views.draw.primitives.Line extends CC.views.draw.primitives.AbstractPrimitives
    constructor:(@start,@end,@name)->
        ...
        if @start instanceof Array
            if @start.length == 2
                if @name?
                    @start = new CC.views.draw.primitives.Point2D(@start[0],@start[1],name + "0",this) 
                else
                    @start = new CC.views.draw.primitives.Point2D(@start[0],@start[1],null,this) 

        if @end instanceof Array
            if @end.length == 2
                if @name?
                    @end = new CC.views.draw.primitives.Point2D(@end[0],@end[1],name + "1",this) 
                else
                    @end = new CC.views.draw.primitives.Point2D(@end[0],@end[1],null,this) 
